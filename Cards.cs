...
namespace PokerBot
{
	public struct Card
	{
		...
		private static readonly string[] HandQualifier = new string[13] { "au 2", "au 3", "au 4", "au 5", "au 6", "au 7", "au 8", "au 9", "au 10", "au Valet", "à la Dame", "au Roi", "à l'As" };
		private static readonly string[] HandQualifiers = new string[13] { "aux 2", "aux 3", "aux 4", "aux 5", "aux 6", "aux 7", "aux 8", "aux 9", "aux 10", "aux Valets", "aux Dames", "aux Rois", "aux As" };
		private static readonly string[] ColorEmojis = new string[4] { "♣️", "♦️", "♥️", "♠️" };
		...
		{
			var couleur = Array.IndexOf(ColorEmojis, str.Substring(str.Length - 2));
			var rang = Array.IndexOf(ShortNames, str.Remove(str.Length - 2));
			if (couleur == -1 || rang == -1) return Card.Invalid;
		...
		public static (int force, string descr) BestHand(IEnumerable<Card> cards)
		{
			...
			if (suite != 0)
			{
				bool flush = lookup[suite].Colors()
					...
					.Any();
				if (flush)
					return suite == 12 ? (80014, "quinte flush royale") : (80002 + suite, "quinte flush " + HandQualifier[suite]);
			}
			var orderedLookup = lookup.OrderByDescending(g => g.Key).ToList();
			var carre = orderedLookup.Find(g => g.Count() == 4)?.Key;
			if (carre.HasValue) return (70002 + carre.Value, "carré " + HandQualifiers[carre.Value]);
			...
			if (brelan.HasValue && paires.Count > 0)
				return (60202 + brelan.Value * 100 + paires[0].Key, "full " + HandQualifiers[brelan.Value] + " et " + HandQualifier[paires[0].Key]);
			var couleur = cards.ToLookup(c => c.Color).Where(g => g.Count() >= 5).ToList();
			if (couleur.Count > 0)
			{
				var rangCouleur = couleur.SelectMany(g => g).Max()._value / 4;
				return (50002 + rangCouleur, "couleur " + HandQualifier[rangCouleur]);
			}
			if (suite != 0)
				return (40002 + suite, "suite " + HandQualifier[suite]);
			if (brelan.HasValue)
				return (30002 + brelan.Value, "brelan " + HandQualifiers[brelan.Value]);
			if (paires.Count >= 2)
			{
				int p1 = paires[0].Key, p2 = paires[1].Key;
				return (20202 + p1 * 100 + p2, "double paire " + HandQualifiers[p1] + " et " + HandQualifiers[p2]);
			}
			if (paires.Count == 1)
				return (10002 + paires[0].Key, "paire " + HandQualifiers[paires[0].Key]);
